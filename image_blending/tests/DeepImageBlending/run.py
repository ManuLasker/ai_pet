...
parser.add_argument('--source_file', type=str, default='data/source_2.jpg', help='path to the source image')
parser.add_argument('--mask_file', type=str, default='data/mask_2.jpg', help='path to the mask image')
parser.add_argument('--target_file', type=str, default='data/target_2.jpg', help='path to the target image')
...
grad_weight = 1e4; style_weight = 1e4; content_weight = 1e3; tv_weight = 1e-6
...
input_img = torch.randn(target_img.shape).to(gpu_id).contiguous()
...
def _pil_image(tensor_image: torch.Tensor):
    if len(tensor_image.shape) > 3:
        tensor_image = np.transpose(tensor_image.squeeze(0).cpu().numpy(),
                                    (1, 2, 0))
    return tensor_image
...
while run[0] <= num_steps:
    ...
    def closure():
        ...
        if run[0] % 50 == 0:
            with torch.no_grad():
                imsave('results/intermediate_first_pass.png', _pil_image(blend_img).astype(np.uint8))
        run[0] += 1
        return loss
...
target_img = np.array(Image.open(target_file).convert('RGB').resize((ts, ts)))
first_pass_img = torch.from_numpy(first_pass_img).unsqueeze(0).transpose(1,3).transpose(2,3).float().to(gpu_id).contiguous()
target_img = torch.from_numpy(target_img).unsqueeze(0).transpose(1,3).transpose(2,3).float().to(gpu_id)
