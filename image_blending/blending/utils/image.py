...
def load_image(path, normalize, is_mask = False):
    if not is_mask:
        if not normalize:
            ...
            image = _to_tensor(Image.open(path).convert("RGB"))
        return image
    else:
        if not normalize:
            ...
            image = _to_tensor(Image.open(path).convert("L"))
        return image
...
def get_laplacian_kernel() -> torch.Tensor:
...
def get_image_laplacian_operator(tensor_image: torch.Tensor) -> Tuple[torch.Tensor,
                                                            torch.Tensor,
                                                            torch.Tensor]:
...
def get_mixing_gradients(image_data:dict, alpha=0.5):
    ...
    rgb_source_gradients = [channel_gradient * mask 
                            for channel_gradient in get_image_laplacian_operator(source)]
